![nvcode.vim](https://raw.githubusercontent.com/joshdick/nvcode.vim/master/img/readme_header.png)
...
![Color Reference](https://raw.githubusercontent.com/joshdick/nvcode.vim/master/img/color_reference.png)

## Installation

1. Install the theme using your Vim plug-in manager of choice (or manually, by placing `colors/nvcode.vim` in your `~/.vim/colors/` directory and `autoload/nvcode.vim` in your `~/.vim/autoload/` directory.)

   The theme also supports being installed as a Vim 8 package. Simply clone this repository into `~/.vim/pack/*/opt/` (so that the local path to this README would end up being `~/.vim/pack/*/opt/nvcode.vim/README.md`) and add `packadd! nvcode.vim` to your `~/.vimrc`. (The `*` in the path can be any value; see `:help packages` for more information.)
...
**Note:** All options should be set **before** the `colorscheme nvcode` line in your `~/.vimrc`.

* `g:nvcode_hide_endofbuffer`: Set to `1` if you want to hide end-of-buffer filler lines (~) for a cleaner look; `0` otherwise (the default).

* `g:nvcode_termcolors` **(see [Troubleshooting](#troubleshooting) (below) for more information about this option)**: Set to `256` for 256-color terminals (the default), or set to `16` to use your terminal emulator's native 16 colors.

* `g:nvcode_terminal_italics`: Set to `1` if your terminal emulator supports italics; `0` otherwise (the default).

## lightline.vim Colorscheme

![lightline-nvcode.vim Preview](https://raw.github.com/joshdick/nvcode.vim/master/img/preview_lightline.png)

This repository includes a companion [lightline.vim](https://github.com/itchyny/lightline.vim) colorscheme for use with nvcode.vim.

The lightline.vim colorscheme:

* Depends on `autoload/nvcode.vim` for its colors, and must therefore be used in conjunction with it.
* Works with both color modes available in nvcode.vim (16 or 256 colors), as specified in the configuration for nvcode.vim.

### Installation

(These instructions assume that lightline.vim and nvcode.vim are already installed and configured to your liking.)

Place `nvcode.vim/autoload/lightline/colorscheme/nvcode.vim` in your `~/.vim/autoload/lightline/colorscheme/` directory either manually or by using your Vim plug-in manager of choice, then add the following lines to your `~/.vimrc` (or merge them into your existing lightline.vim configuration):
...
![airline-nvcode.vim Preview](https://raw.github.com/joshdick/nvcode.vim/master/img/preview_airline.png)

This repository includes a companion [vim-airline](https://github.com/vim-airline/vim-airline) theme for use with nvcode.vim.
...
(These instructions assume that vim-airline and nvcode.vim are already installed and configured to your liking.)

Place `nvcode.vim/autoload/airline/themes/nvcode.vim` in your `~/.vim/autoload/airline/themes/` directory either manually or by using your Vim plug-in manager of choice, then add the following line to your `~/.vimrc`:
...
![Broken Colors](https://raw.githubusercontent.com/joshdick/nvcode.vim/master/img/broken_colors.png)
...
However, you can use the `g:nvcode_termcolors` option to control nvcode.vim's fallback colors in situations where 24-bit color terminal support is not available.

`g:nvcode_termcolors` can be set to one of two values, `256` for 256-color mode (the default), or `16` for 16-color mode (which uses your terminal emulator's native 16 colors.)

**When Vim/Neovim are configured to use 24-bit color, the `g:nvcode_termcolors` option is ignored. However, `g:nvcode_termcolors` can still coexist with 24-bit color configuration.** For instance, when tmux 2.1 or earlier and/or older Vim/Neovim versions that don't support 24-bit color are used, the `g:nvcode_termcolors` option will take effect.
...
* **16-color mode** is the preferred option, since its colors are more accurate than those of 256-color mode. However, you'll need to set your terminal emulator's color palette to this color scheme's custom 16-color palette, since 16-color mode will cause the color scheme to use your terminal emulator's native 16 colors. If you don't use the custom 16-color palette in your terminal emulator when 16-color mode is enabled, onedark.vim's colors will not display correctly in Vim.
   ...
   ```vim
   let g:nvcode_termcolors=16
   ```

### Why do all comments look like they're highlighted?

![Broken Italics](https://raw.githubusercontent.com/joshdick/nvcode.vim/master/img/broken_italics.png)
...
Make sure that in Preferences, Profiles -> [Active Profile] -> Colors -> Minimum Contrast is turned all the way down. See [#145](https://github.com/joshdick/nvcode.vim/issues/145) for more information.

## Miscellaneous

### Customizing nvcode.vim's look without forking the repository

nvcode.vim exposes `nvcode#extend_highlight` and `nvcode#set_highlight` functions that you can call from within your `~/.vimrc` in order to customize the look of nvcode.vim.

#### `nvcode#extend_highlight`

`nvcode#extend_highlight` allows you to customize individual aspects of nvcode.vim's existing highlight groups, overriding only the keys you provide. (To completely redefine/override an existing highlight group, see `nvcode#set_highlight` below.)

`nvcode#extend_highlight`'s first argument should be the name of a highlight group, and its second argument should be **partial** style data.

Place the following lines **before** the `colorscheme nvcode` line in your `~/.vimrc`, then change the example overrides to suit your needs:
...
if (has("autocmd"))
  augroup colorextend
    ...
    autocmd ColorScheme * call nvcode#extend_highlight("Function", { "gui": "bold" })
    " Override the `Statement` foreground color in 256-color mode
    autocmd ColorScheme * call nvcode#extend_highlight("Statement", { "fg": { "cterm": 128 } })
    " Override the `Identifier` background color in GUI mode
    autocmd ColorScheme * call nvcode#extend_highlight("Identifier", { "bg": { "gui": "#333333" } })
  augroup END
endif
```

#### `nvcode#set_highlight`

`nvcode#set_highlight` allows you to completely redefine/override highlight groups of your choosing.

`nvcode#set_highlight`'s first argument should be the name of a highlight group, and its second argument should be **complete** style data.

For example, to remove the background color only when running in terminals (outside GUI mode and for use in transparent terminals,) place the following lines **before** the `colorscheme nvcode` line in your `~/.vimrc`:
...
You can override colors across all highlights by adding color definitions to the `g:nvcode_color_overrides` dictionary in your `~/.vimrc` like so:

```vim
let g:nvcode_color_overrides = {
\ "black": {"gui": "#2F343F", "cterm": "235", "cterm16": "0" },
\ "purple": { "gui": "#C678DF", "cterm": "170", "cterm16": "5" }
\}
```

This also needs to be done **before** `colorscheme nvcode`.

More examples of highlight group names and style data can be found in nvcode.vim's source code (`colors/nvcode.vim` inside this repository).

### tmux theme

If you'd like a tmux theme that complements nvcode.vim, [@odedlaz has you covered](https://github.com/odedlaz/tmux-nvcode-theme).

### Contributing

If you'd like to contribute to nvcode.vim, check out the [contribution guidelines](./CONTRIBUTING.md).

### Relatives of nvcode.vim

Several other themes and projects have reused code and/or colors from this project, or were otherwise inspired by it.

If nvcode.vim isn't meeting your needs, try one of its relatives!
...
* [base16-nvcode.vim](https://github.com/chriskempson/base16-vim/blob/master/colors/base16-nvcode.vim)
  * Associated base16 scheme: [tilal6991/base16-nvcode-scheme](https://github.com/tilal6991/base16-nvcode-scheme)
